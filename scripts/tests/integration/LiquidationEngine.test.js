    ...
    describe("#liquidate", async () => {
        context("price drop but does not make the position underwater", async () => {
            it("should revert", async () => {
                ...
                await expect(
                    liquidationEngine["liquidate(bytes32,address,uint256,uint256,address,bytes)"](pools.XDC, alicePositionAddress, 1, 1, AliceAddress, "0x", { gasLimit: 3000000 })
        ...
        context("main liquidation scenarios", async () => {
            ...
            for (let i = 0; i < testParams.length; i++) {
                ...
                it(testParam.label, async () => {
                    ...
                    await collateralPoolConfig.setLiquidatorIncentiveBps(pools.XDC, testParam.liquidatorIncentiveBps)
                    await collateralPoolConfig.setCloseFactorBps(pools.XDC, testParam.closeFactorBps)
                    ...
                    await collateralPoolConfig.setLiquidationRatio(pools.XDC, ratio, { gasLimit: 3000000 })
                    await collateralPoolConfig.setDebtFloor(pools.XDC, parseUnits(testParam.debtFloor, 45), { gasLimit: 1000000 })
                    ...
                    await PositionHelper.openXDCPositionAndDraw(aliceProxyWallet, AliceAddress, pools.XDC, lockedCollateralAmount, drawStablecoinAmount);
                    const alicePositionAddress = await positionManager.positions(1)
                    ...
                    const alicePositionAfterLiquidation = await bookKeeper.positions(pools.XDC, alicePositionAddress)
                    ...
                    AssertHelpers.assertAlmostEqual(
                        (await bookKeeper.collateralToken(pools.XDC, systemDebtEngine.address)).toString(),
        ...
        context("1st liquidation keep position unsafe, 2nd position fully liquidate the position", async () => {
            ...
            it(testParam.label, async () => {
                await fathomStablecoin.approve(fixedSpreadLiquidationStrategy.address, MaxUint256, { from: BobAddress, gasLimit: 1000000 })
                await fathomStablecoin.mint(BobAddress, parseUnits(testParam.debtShareToRepay, 47), { gasLimit: 1000000 })
                ...
                await priceOracle.setPrice(pools.XDC);
                let ratio = WeiPerRay.mul(1000).div(parseUnits(testParam.collateralFactor, 3))
                await collateralPoolConfig.setLiquidationRatio(pools.XDC, ratio, { gasLimit: 1000000 })
                await collateralPoolConfig.setDebtFloor(pools.XDC, parseUnits(testParam.debtFloor, 45), { gasLimit: 1000000 })
                ...
                const alicePositionAfterLiquidation2 = await bookKeeper.positions(pools.XDC, alicePositionAddress)
                expect(alicePositionAfterLiquidation2.lockedCollateral).to.be.eq(parseEther("5.38"))
        ...
        context(
            ...
            async () => {
                it("should success", async () => {
                    ...
                    await collateralPoolConfig.setLiquidationRatio(pools.XDC, ratio, { gasLimit: 1000000 })
                    //   await collateralPoolConfig.setPriceWithSafetyMargin(pools.XDC, parseUnits("294", 27), { gasLimit: 1000000 })
                    await collateralPoolConfig.setDebtFloor(pools.XDC, parseEther("100").mul(WeiPerRay), { gasLimit: 1000000 })
                    ...
                    const aliceDebtValueAfterOneYear = (
                        await bookKeeper.positions(pools.XDC, alicePositionAddress)
                    ).debtShare.mul((await collateralPoolConfig.collateralPools(pools.XDC)).debtAccumulatedRate)
                    AssertHelpers.assertAlmostEqual(
                    ...
                    expect(
                        ...
                        "debtShare should be 1000 FXD, because Bob liquidated 1000 FXD from Alice's position"
                    )
                    ...
                    expect(
                        ...
                        "System bad debt should be 0 FXD"
                    ).to.be.equal(0)
                    ...
                    ) // Bob should pay 1005 FXD for this liquidation
                    AssertHelpers.assertAlmostEqual(
                        (await bookKeeper.collateralToken(pools.XDC, systemDebtEngine.address)).toString(),
        ...
        context("batch liquidation", async () => {
            ...
            it(testParam.label, async () => {
                ...
                const alicePosition = await bookKeeper.positions(pools.XDC, alicePositionAddress1)
                ...
                const alicePositionAfterLiquidation = await bookKeeper.positions(pools.XDC, alicePositionAddress1)
                const expectedSeizedCollateral = parseUnits(testParam.expectedSeizedCollateral, 18)
                const expectedLiquidatorIncentive = expectedSeizedCollateral.sub(
                    expectedSeizedCollateral.mul(BPS).div(testParam.liquidatorIncentiveBps)
                )
